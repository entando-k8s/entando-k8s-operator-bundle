...
function writeAllResourcesForVersion {
  ...
  helm template --set operator.supportOpenshift=false,operator.clusterScope=false,bundle.olmDisabled=true ./  >> ${output_dir}/namespace-resources.yaml
  writeClusterResourceToFile ${k8s_version} ${output_dir}/all-in-one.yaml
  helm template --set operator.supportOpenshift=false,operator.clusterScope=false,bundle.olmDisabled=true ./  >> ${output_dir}/all-in-one.yaml
  output_dir=./manifests/${k8s_version}/cluster-scoped-deployment
  rm $output_dir/*.yaml 2>/dev/null
  writeClusterResourceToFile ${k8s_version} ${output_dir}/all-in-one.yaml
  helm template --set operator.supportOpenshift=false,operator.clusterScope=true,bundle.olmDisabled=true ./  >> ${output_dir}/all-in-one.yaml
}
