#!/usr/bin/env bash
#This script determines the latest version of the entando-k8s-operator-bundle-community container that was created
...
#(Dockerfile.community) has been rebuilt as a result of a PR being merged into master.
...
for V in ${PREVIOUS_VERSIONS[@]}; do
  BUNDLES="${BUNDLES}docker.io/entando/entando-k8s-operator-bundle-community:${V},"
done
BUNDLES="${BUNDLES}docker.io/entando/entando-k8s-operator-bundle-community:${MY_VERSION}"
echo $BUNDLES
operator-sdk bundle validate docker.io/entando/entando-k8s-operator-bundle-community:${MY_VERSION} \
